...
Jemalloc can be disabled by passing environmenatal variable `DISABLE_JEMALLOC` with non-null value, e.g `-e "JEMALLOC_DISABLE=true"`

Reason for disabling Jemalloc can be compatiblity issues on specific hardware, for example Docker container failing with  `<jemalloc>: Unsupported system page size` error message.
